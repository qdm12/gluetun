...
type Configurator interface {
	...
	SetVPNPortRedirection(ctx context.Context, src, dst uint16) (err error)
	RemoveVPNPortRedirection(ctx context.Context, src uint16) (err error)
	SetDebug()
...
type configurator struct { //nolint:maligned
	...
	vpnPortRedirection map[uint16]uint16 // src -> dst ports
...
func NewConfigurator(logger logging.Logger, routing routing.Routing, fileManager files.FileManager) Configurator {
	return &configurator{
		...
		vpnPortRedirection: map[uint16]uint16{},
	}
}
