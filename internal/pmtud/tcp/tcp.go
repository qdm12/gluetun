...
func runTest(ctx context.Context, fd fileDescriptor,
	...
	packetType, synAckSeq, synAckAck, err := parseTCPHeader(reply[:constants.TCPHeaderLength])
	switch {
	case err != nil:
	...
	packetType, _, ack, err := parseTCPHeader(reply[:constants.TCPHeaderLength])
	if err != nil {
...
func handleRSTReply(ctx context.Context, fd fileDescriptor, ch <-chan []byte,
	...
	packetType, _, _, err := parseTCPHeader(reply[:constants.TCPHeaderLength])
	if err != nil {
