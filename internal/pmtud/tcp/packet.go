...
func createSYNPacket(src, dst netip.AddrPort, mtu uint32) (packet []byte, seq uint32) {
	...
	payloadLength := constants.TCPHeaderLength // no data payload
	if mtu > 0 {
...
func createRSTPacket(src, dst netip.AddrPort, seq, ack uint32) []byte {
	const payloadLength = constants.TCPHeaderLength // no data payload
...
func getPayloadLength(mtu uint32, dst netip.AddrPort) uint32 {
	...
	if mtu < ipHeaderLength+constants.TCPHeaderLength {
...
func createPacket(src, dst netip.AddrPort,
	...
	if payloadLength < constants.TCPHeaderLength {
	...
	// data is just zeroes
	dataLength := int(payloadLength) - int(constants.TCPHeaderLength)
	var data []byte
	if dataLength > 0 {
	...
	packet := make([]byte, len(ipHeader)+int(constants.TCPHeaderLength)+dataLength)
	copy(packet, ipHeader)
	copy(packet[len(ipHeader):], tcpHeader)
	copy(packet[len(ipHeader)+int(constants.TCPHeaderLength):], data)
	return packet
}
