...
func setupWireguard(ctx context.Context, netlinker NetLinker,
	...
	wireguarder *wireguard.Wireguard, extra tunnelUpData, err error) {
	connection, err := providerConf.GetConnection(settings.Provider.ServerSelection, ipv6Supported)
	if err != nil {
		return nil, tunnelUpData{}, fmt.Errorf("finding a VPN server: %w", err)
	}
	...
	if err != nil {
		return nil, tunnelUpData{}, fmt.Errorf("creating Wireguard: %w", err)
	}

	err = fw.SetVPNConnection(ctx, connection, settings.Wireguard.Interface)
	if err != nil {
		return nil, tunnelUpData{}, fmt.Errorf("setting firewall: %w", err)
	}

	extra = tunnelUpData{
		serverHostname: connection.Hostname,
		canPortForward: connection.PortForward,
		vpnIntf:        settings.Wireguard.Interface,
	}
	return wireguarder, extra, nil
}
