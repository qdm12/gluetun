...
func (p *piaV4) PortForward(ctx context.Context, client *http.Client,
	...
	if err := fw.SetAllowedPort(ctx, data.Port, tun); err != nil {
		pfLogger.Error(err)
	}
	if err := fw.SetVPNPortRedirection(ctx, data.Port, redirectionPort); err != nil {
		pfLogger.Error(err)
	}
	...
	for {
		...
		case <-ctx.Done():
			...
			if err := fw.RemoveVPNPortRedirection(ctx, data.Port); err != nil {
				pfLogger.Error(err)
			}
			return
		...
		case <-expiryTimer.C:
			...
			if err := fw.RemoveVPNPortRedirection(ctx, oldPort); err != nil {
				pfLogger.Error(err)
			}
			if err := fw.SetAllowedPort(ctx, data.Port, tun); err != nil {
				pfLogger.Error(err)
